...
# @TEST-EXEC: cat logschema.log| jq '. | select(.name=="conn")' | jq -cr .schema | jq . | grep -v 'at this installation' > schema.json
# @TEST-EXEC: btest-diff schema.json
# @TEST-EXEC: jq . schema.json
